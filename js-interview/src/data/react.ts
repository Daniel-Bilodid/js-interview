...
function Profile() {
  ...
  },
  {
    ...
    description:
      'Основні три: useState — локальний стан компонента; повертає саме значення й сеттер, виклик якого запускає ререндер. useEffect — побічні ефекти після рендеру (запити, підписки, таймери); масив залежностей вирішує, коли ефект повториться, а повернена функція прибирає за собою. useContext — читає значення найближчого Provider без прокидання пропсів через усе дерево. Додаткові: useReducer — той самий стан, але коли переходів багато й вони повʼязані, логіка виноситься в reducer(state, action); useRef — мутабельний контейнер .current, який переживає рендери й НЕ викликає ререндер (а ще посилання на DOM-вузол); useMemo — кешує результат важкого обчислення між рендерами; useCallback — кешує саму функцію, щоб не ламати memo у дочірніх компонентів; useLayoutEffect — як useEffect, але синхронно до перемальовування, коли треба виміряти DOM без «мигання»; useImperativeHandle — налаштовує, що саме батько отримає через ref на ваш компонент. Сучасні (React 18+): useTransition — позначає оновлення як нетермінове, щоб важкий ререндер не блокував ввід, і дає прапорець isPending; useDeferredValue — те саме, але для одного значення: віддає його «відкладену» копію, поки готується нова; useId — стабільний унікальний id, однаковий на сервері й клієнті; useSyncExternalStore — безпечна підписка на зовнішнє сховище; useInsertionEffect — вузький хук для CSS-in-JS, вставляє стилі до розрахунку layout. React 19 додав: use — читає проміс або контекст прямо в рендері (працює разом із Suspense); useActionState — стан форми навколо асинхронної дії; useOptimistic — показує результат оптимістично, до відповіді сервера; useFormStatus — статус відправки форми в дочірньому компоненті. Плюс кастомні хуки — власні функції з префіксом use, що комбінують стандартні хуки для перевикористання логіки.',
    codeExamples: [
      {
        label: 'Найуживаніші',
